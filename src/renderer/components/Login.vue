      ...
      <v-container>
        <div class="login-wrapper">
          ...
          <v-card-text>
            <v-layout row wrap>
              <v-flex xs12 class="mb-2">
                <v-text-field hide-details label="用户名" v-model="username"></v-text-field>
                <div class="verror">
                  <span v-if="$v.username.$invalid && $v.$dirty">请输入用户名</span>
                </div>
              </v-flex>
              <v-flex xs12 class="mb-4">
                <v-text-field hide-details label="密码" v-model="password" type="password"></v-text-field>
                <div class="verror">
                  <span v-if="$v.password.$invalid && $v.$dirty">请输入密码</span>
                </div>
              </v-flex>
              <v-flex xs11>
          ...
          <v-card-actions class="pr-2">
            <v-spacer></v-spacer>
            <v-btn type="submit" color="primary" dark @click="onLogin" :loading="loggingIn">登录</v-btn>
          </v-card-actions>
          <v-dialog v-model="noserver" max-width="300">
            <v-card>
              <v-card-title class="headline">实例不可用</v-card-title>
              <v-card-text>无法联系指定的服务器位置。确认您已经启动了一个站点，其中实例和微服务已成功启动。</v-card-text>
              <v-card-actions>
                <v-spacer></v-spacer>
